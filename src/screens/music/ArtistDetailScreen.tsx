...
import { useLibraryStore, getArtistsFromSongs } from '../../store/libraryStore';
import { usePlayerStore } from '../../store/playerStore';

type Nav = NativeStackNavigationProp<MusicStackParamList>;
type R = RouteProp<MusicStackParamList, 'ArtistDetail'>;

export function ArtistDetailScreen(): React.ReactElement {
  const navigation = useNavigation<Nav>();
  const route = useRoute<R>();
  const songs = useLibraryStore((s) => s.songs);
  const artist = useMemo(
    () => getArtistsFromSongs(songs).find((a) => a.id === route.params.artistId),
    [songs, route.params.artistId],
  );
  const list = useMemo(
    () => (artist ? songs.filter((s) => artist.songIds.includes(s.id)) : []),
    [artist, songs],
  );
  ...
  return (
    <View style={styles.root}>
      ...
      <Text style={[Typography.hero, styles.t]}>{artist.name}</Text>
      <FlashList
        data={list}
